...
export const DailyWirdView: React.FC<DailyWirdViewProps> = ({ onNavigate }) => {
  ...
  const wisdomItem = wird.items.find((i) => i.type === 'wisdom');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 pb-12" dir="rtl">
      ...
      {/* ================================================================ */}
      {/* 6. SECTION: WISDOM & REMINDER (موعظة وتذكير اليوم) */}
      {/* ================================================================ */}
      {wisdomItem && wisdomItem.wisdomData && (
        <section
          aria-labelledby="section-wisdom-title"
          className="rounded-3xl p-6 sm:p-7 bg-white dark:bg-emerald-950/40 border border-slate-200/80 dark:border-emerald-800/40 shadow-sm text-right"
        >
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-amber-500/15 text-amber-700 dark:bg-amber-400/20 dark:text-amber-300 flex items-center justify-center">
              ...
              <div>
                <span className="text-xs font-bold text-amber-600 dark:text-amber-400 block">
                  {wisdomItem.wisdomData.contentType || 'تذكير اليوم'}
                </span>
                <h2 id="section-wisdom-title" className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">
                  {wisdomItem.titleAr || 'موعظة وتذكير اليوم'}
                </h2>
              </div>
            </div>

            <span className="px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-300 font-bold text-xs">
              {wisdomItem.wisdomData.category}
            </span>
          </div>

          <div className="p-5 sm:p-6 rounded-2xl bg-amber-50/40 dark:bg-emerald-900/20 border border-amber-500/10 dark:border-emerald-700/30 text-slate-900 dark:text-slate-100 font-amiri text-lg sm:text-xl leading-relaxed text-center tracking-wide my-4 select-text">
            {wisdomItem.wisdomData.content}
          </div>

          {wisdomItem.wisdomData.author && (
            <p className="text-center text-xs font-bold text-emerald-700 dark:text-emerald-400 mb-2">
              — {wisdomItem.wisdomData.author} —
            </p>
          )}
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400 mb-4">
            <p>
              <strong className="text-slate-700 dark:text-slate-300">المصدر: </strong>
              {wisdomItem.wisdomData.source} {wisdomItem.wisdomData.reference ? `(${wisdomItem.wisdomData.reference})` : ''}
            </p>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-emerald-900/30">
            <button
              onClick={() =>
                openShareModal({
                  ...
                  contentType: wisdomItem.wisdomData?.contentType || 'حكمة وموعظة',
                  content: wisdomItem.wisdomData?.content,
                  text: wisdomItem.wisdomData?.content,
                  source: wisdomItem.wisdomData?.author ? `${wisdomItem.wisdomData.author} — ${wisdomItem.wisdomData.source}` : wisdomItem.wisdomData?.source,
                  title: wisdomItem.titleAr || 'تذكير اليوم',
                })
              }
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-emerald-900/40 transition-colors"
            >
            ...
            <button
              onClick={() =>
                toggleFavorite({
                  type: 'wisdom',
                  referenceId: wisdomItem.wisdomData?.wisdomId || wisdomItem.id,
                  title: wisdomItem.titleAr || 'تذكير اليوم',
                  subtitle: wisdomItem.wisdomData?.source,
                  contentSnippet: wisdomItem.wisdomData?.content?.substring(0, 75),
                })
              }
              className={`p-2 rounded-xl transition-colors ${
                isFavorite('wisdom', wisdomItem.wisdomData?.wisdomId || wisdomItem.id)
                  ? 'text-rose-500 bg-rose-50 dark:bg-rose-950/40'
                  : 'text-slate-400 hover:text-rose-500 hover:bg-slate-100 dark:hover:bg-emerald-900/30'
              }`}
              title="إضافة للمفضلة"
